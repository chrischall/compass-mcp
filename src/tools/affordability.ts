 ...
 * the standard 28/36 DTI rule. Same math used by zillow-mcp and
 * redfin-mcp.
 */
...
export function computeAffordability(input: {
  ...
  // Solve max loan amount: PITI = PI(loan) + (price * taxRate)/12 + ins + hoa
  // and price = loan + down. Substitute and solve for loan.
  // PI(loan) ≈ loan * factor, where factor = monthly factor for term + rate.
  ...
  // PITI = loan * piFactor + tax(loan) + ins + hoa
  //      = loan * piFactor + (loan + down) * taxRate/12 + ins + hoa
  // → loan * (piFactor + taxRate/12) + down * taxRate/12 + ins + hoa = maxPiti
  // → loan = (maxPiti - down*taxRate/12 - ins - hoa) / (piFactor + taxRate/12)
